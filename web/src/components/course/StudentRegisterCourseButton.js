            ...
            unregisterCourse({
              ...
              .then(() => {
                ...
                notification.success({
                  message: `Bạn đã huỷ học phần ${course.title}`,
                });
                props.onDone();
              })
              .catch((err) => {
                ...
                if (err.toString().includes("course is closed")) {
                  notification.error({
                    message: `có lỗi xảy ra: học phần này đã khoá`,
                  });
                } else {
                  notification.error({
                    message: `có lỗi xảy ra: ${err.toString()}`,
                  });
                }
              });
          }}
            ...
            registerCourse({
              ...
              .catch((err) => {
                ...
                if (err.toString().includes("course is overlap")) {
                  notification.error({
                    message: `có lỗi xảy ra: trùng lịch học`,
                  });
                } else if (err.toString().includes("course is closed")) {
                  notification.error({
                    message: "Học phần này đã dừng đăng ký",
                  });
                } else if (
                  err
                    .toString()
                    .includes("this course is not open for your faculty")
                ) {
                  notification.error({
                    message: "Học phần này không đành cho khoa của bạn",
                  });
                } else {
                  notification.error({
                    message: `có lỗi xảy ra: ${err.toString()}`,
