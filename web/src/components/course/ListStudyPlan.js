...
import {Button, Col, Drawer, Form, Input, InputNumber, notification, Row, Select, Table, Tag} from "antd";
...
import AdminStudyPlan from "../../pages/admin/AdminStudyPlan";
                    ...
                    <Drawer
                        ...
                        <Form
                            initialValues={editCourse ? {
                                ...
                                faculties: editCourse.faculties ? editCourse.faculties.map((f) => f.id) : []
                            } : null}
                            layout={"vertical"}
                            onFinish={(values) => {
                                setLoading(true)
                            ...
                            <Form.Item label={"Loại học phần"} name={"required"}
                            ...
                            <Form.Item label={'Số tín chỉ'} name={"unit"}
                            ...
                            <Row>
                                <Col span={8}>
                                    <Form.Item label={"Tiết bắt đầu"} name={"lessonFrom"}
                                               rules={[{required: true, message: "Chọn tiết bắt đầu"}]}>
                                        <InputNumber min={1}/>
                                    </Form.Item>
                                </Col>
                                <Col span={8}>
                                    <Form.Item label={"Tiết kết thúc"} name={"lessonTo"}
                                               rules={[{required: true, message: "Chọn tiết kết thúc"}]}>
                                        <InputNumber min={1}/>
                                    </Form.Item>
                                </Col>
                                <Col span={8}>
                                    <Form.Item label={"Giới hạn đăng ký"} name={"limit"}
                            ...
                            <Form.Item label={"Giảng viên"} name={'teacherId'}
                                       rules={[{required: true, message: "Chọn giảng viên"}]}>
                                <TeacherSelection/>
                            </Form.Item>
                            <Form.Item label={"Khoa"} name={'faculties'}
                                       rules={[{required: true, message: "Chọn khoa"}]}>
                                <FacultySelection mode={"multiple"}/>
                            </Form.Item>
                            <Form.Item label={"Tình trạng"} name={"open"}
                                       rules={[{required: true, message: "Chọn tình trạng mở hay đóng"}]}>
                                <Select>
                            ...
                            <div className={"submit"}>
                                <Button loading={isLoading}
                                        htmlType={'submit'}>{editCourse ? 'Lưu thông tin' : 'Mở học phần'}</Button>
                            </div>
                        </Form>
                    </Drawer>
                )
